...
//			dispatcher.forward(req, resp);
		...
		if(loginMember != null) { // 로그인 성공 시
			...
			session.setMaxInactiveInterval(60 * 60); // 60초 * 60분 1시간(3600초) == 로그인 후 1시간 뒤 세션 만료(로그아웃)
			...
			if(req.getParameter("saveId") != null) { // 체크된 경우
				...
				cookie.setMaxAge(60*60*24*30); // 30일동안 살아라
				
			}else{ // 체크가 안 된 경우
				
				
				// 4) 쿠기의 유효시간을 0초로 설정
				// ==> 클라이언트 저장된 savaId 쿠키를 삭제하라는 의미 (0초만 살아라 = 살지마라)
