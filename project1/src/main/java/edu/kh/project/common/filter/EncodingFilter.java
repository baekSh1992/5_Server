 ...
 *   							객체 생성
 *   
 *   - 여러 필터를 만들어 연쇄적으로 수행 가능. (FilterChain = 사슬처럼 엮여있다)
 *   
 *   - 만약, 필터 순서를 지정하고자 하면 
...
//						   + 필터로 등록
@WebFilter(filterName = "encodingFilter", urlPatterns = "/*") // [ "/*" = 최상위 주소 아래 모든 ] 
public class EncodingFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		// place your code here (당신이 원하는 코드를 쓰는 곳이다)
		
		// ServletRequest  == HttpServletRequest  의  부모 인터페이스
		// ServletResponse == HttpServletResponse 의  부모 인터페이스 
		...
		// pass the request along the filter chain (원하는 요청으로 필터를 넘긴다)
